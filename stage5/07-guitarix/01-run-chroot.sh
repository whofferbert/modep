...
pushd ${TMP_DIR}/trunk
./waf configure --lv2dir=${LV2_DIR} --lv2-only --no-lv2-gui --disable-sse --no-faust
./waf build -j4 # -vvv
./waf -j1 install
popd
...
# Does not work.
rm -rf ${LV2_DIR}/gx_mbcompressor.lv2

# Remove modgui folders so that mod-lv2-data gets copied over.
rm -r ${LV2_DIR}/gxts9.lv2/modgui
rm -r ${LV2_DIR}/gxmetal_head.lv2/modgui
rm -r ${LV2_DIR}/gxmetal_amp.lv2/modgui
